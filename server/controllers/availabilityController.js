...
availabilityController.getSubjectID = (req, res, next) => {
  ...
  db.query(sqlQuery, subjectWord)
    ...
    .catch((err) => {
      return next({
        log:
          "availabiltyController.getSubjectID: ERROR: Error getting skill data from the database",
        message: {
          err:
            "availabiltyController.getSubjectID: ERROR: Check server logs for details",
        },
      });
    });
};
...
availabilityController.getTutors = (req, res, next) => {
  ...
  db.query(sqlQuery, subjectID)
    ...
    .catch((err) => {
      return next({
        log:
          "availabiltyController.getTutor: ERROR: Error getting tutor data from the database",
        message: {
          err:
            "availabiltyController.getTutor: ERROR: Check server logs for details",
        },
      });
    });
};
